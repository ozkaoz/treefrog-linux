# Build (FASE C — guía de reproducibilidad)
...
# 1. Obtener Linux 4.4.186 vanilla (usa tarball local verificado; sha256 check)
scripts/fetch-kernel.sh

# 2. Aplicar el flujo Hichip real:
#    38 patches + rsync linux-drivers + yaffs2 patch-ker
scripts/apply-patches.sh

# 3. Compilar (board de referencia del SDK para smoke test de compilación):
scripts/build-kernel.sh hc16xx-db-a3100-v10 squashfs
# genera out/hc16xx-db-a3100-v10/: vmlinux.bin (gzip+DTB appended), dtb.bin, vmlinux, manifest.json
```
...
| DTB | build-kernel.sh | make dtbs (hcboot también usa CONFIG_CUSTOM_DTS_PATH) |
| vmlinux gzip + DTB append | build-kernel.sh | linux-ext-elf-append-dtb.mk |
...
2. Los blobs `.o` vendor (hc-p2p, get_adc_default_val*) NO se importaron al repo (licencia de redistribución dudosa). El rsync del paso 2 los copia desde vendor/ si están; si el config de la board no activa HC_P2P, no se enlazan. Para activarlos: copiarlos del SDK local y documentarlo.
